...
{
    public static class ConnectionST
    {
        public static string GetConnectionString(string connectioName,string salt = "",string envFilePath = @"\\APPSERVER29\cmisenv$\env.cfg")
        {
            try
            {
                var env = CmisEnvFile.Env(envFilePath);
                return Crypto.Decrypt(env[connectioName], salt);
            }
            catch (Exception ex)
            {
                throw ex;
